import numpy as np
import struct
from lib.datasets.tless import opengl_backend
...
def render(model, pose, K, w, h, render_type='depth'):
    R = pose[:, :3]
    t = pose[:, 3:] * 1000.
    if render_type == 'depth':
        return opengl_backend.render(model, im_size=[w, h], K=K, R=R, t=t, clip_near=10, clip_far=10000, mode='depth') / 1000.
    else:
        return opengl_backend.render(model, im_size=[w, h], K=K, R=R, t=t, clip_near=10, clip_far=10000, mode='rgb')
