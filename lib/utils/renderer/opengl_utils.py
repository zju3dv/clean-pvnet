...
class DepthRender:
    def __init__(self, model, im_size,
               texture=None, bg_color=(0.0, 0.0, 0.0, 0.0),
               ambient_weight=0.5, shading='flat', mode='rgb+depth'):
        ...
        self.index_buffer = model['faces'].flatten().astype(np.uint32).view(gloo.IndexBuffer)
        self.window = app.Window(visible=False)

        self.program = gloo.Program(_depth_vertex_code, _depth_fragment_code)
    ...
    def draw_depth(self, shape, vertex_buffer, index_buffer, texture, mat_model, mat_view,
                   mat_proj, ambient_weight, bg_color, shading):
        self.program['u_mv'] = _compute_model_view(mat_model, mat_view)
        self.program['u_mvp'] = _compute_model_view_proj(mat_model, mat_view, mat_proj)
    ...
    def render(self, im_size, clip_near, clip_far, K, R, t):
        # Model matrix
        mat_model = np.eye(4, dtype=np.float32) # From object space to world space
        ...
        def on_draw(dt):
            ...
            # Render color image
            global depth
            depth = self.draw_depth(shape, self.vertex_buffer, self.index_buffer, None, mat_model,
                                    mat_view, mat_proj, 0.5, bg_color=(0.0, 0.0, 0.0, 0.0),
                                    shading='flat')
...
class NormalRender:
    ...
    def draw_depth(self, shape, vertex_buffer, index_buffer, texture, mat_model, mat_view,
                   mat_proj, ambient_weight, bg_color, shading):
        self.program['u_mv'] = _compute_model_view(mat_model, mat_view)
        self.program['u_nm'] = _compute_normal_matrix(mat_model, mat_view)
        self.program['u_mvp'] = _compute_model_view_proj(mat_model, mat_view, mat_proj)
        ...
        depth.shape = shape[0], shape[1], 4
        depth = depth[::-1, :]
        # depth = depth[:, :, :3] # Depth is saved in the first channel
        rgb = np.round(depth[:, :, :3] * 255).astype(np.uint8) # Convert to [0, 255]
